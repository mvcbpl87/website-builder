...
import { testFunnel, testFunnelPage } from '@/test/dummy/type'

type Props = {
  funnel: testFunnel
  subaccountId: string
  pages: testFunnelPage[]
  funnelId: string
}

const FunnelSteps = ({ funnel, funnelId, pages, subaccountId }: Props) => {
  const [clickedPage, setClickedPage] = useState<testFunnelPage | undefined>(
    pages[0]
  )
        ...
        <aside className="flex-[0.7] bg-muted p-4 ">
          {!!pages.length ? (
            <Card className="h-full flex justify-between flex-col">
              <CardHeader>
                <p className="text-sm text-muted-foreground">Page here</p>
